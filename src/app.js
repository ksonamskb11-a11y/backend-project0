...
app.use(urlencoded({limit:"16kb"})); // tto make understand 'url-code' to backend 
...
// to create API, by using '.get'
app.get('/', (req, res) => {
  res.send('Hello World...!!!  Welcome to BackEnd...!!!');     // here we can send only one response at a time 
//   res.send(data);
//   res.send({origin:process.env.ORIGIN});
})
...
//    const pass  = req.body;
  ...
  try {
        ...
        throw new ApiError(404 , "** Server Not Found / Invalid ID **",error);
       } 
})

// set routes to use get perticular data and also this gives output data as- 'only a single person'
app.get('/persons/:id', (req, res)=>{              // "http://localhost:4000/persons/9" use to check it's work or not
  const id = parseInt(req.params.id);
  const singlePerson = data.find((p)=> p.id === id)
  if(singlePerson){
